...
def classify(hand: str) -> int:
    return max(map(score, replace_jokers(hand)))


def replace_jokers(hand: str) -> list[str]:
    if hand == "":
        return [""]

    return [
        x + y
        for x in (CARD_RANKS[1:] if hand[0] == "J" else hand[0])
        for y in replace_jokers(hand[1:])
    ]
